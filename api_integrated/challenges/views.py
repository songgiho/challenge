...
class ChallengeViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        """챌린지 생성 시 생성자를 자동으로 참가자로 등록, 칼로리 챌린지 중복 생성 방지"""
        user = request.user
        # 칼로리 챌린지 중복 생성 방지
        target_type = request.data.get('targetType') or request.data.get('target_type')
        if target_type == 'calorie':
            from .models import ChallengeParticipant, Challenge
            active_calorie = ChallengeParticipant.objects.filter(
                user=user,
                status='survived',
                challenge__is_active=True,
                challenge__target_type='calorie'
            ).exists()
            if active_calorie:
                return Response({
                    'success': False,
                    'error': '이미 참여 중인 칼로리 챌린지가 있습니다. 한 번에 하나만 생성/참여할 수 있습니다.',
                    'status_code': 400
                }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def join(self, request, pk=None):
        ...
        if challenge.target_type == 'calorie':
            ...
            if active_calorie:
                return Response({
                    'success': False,
                    'error': '이미 참여 중인 칼로리 챌린지가 있습니다. 한 번에 하나만 참여할 수 있습니다.',
                    'status_code': 400
                }, status=status.HTTP_400_BAD_REQUEST)
        # 기간 중복 확인
    ...
    def my_challenges(self, request):
        """현재 로그인한 사용자의 챌린지 목록 반환"""
        user = request.user
        challenges = Challenge.objects.filter(participants__user=user).distinct()
