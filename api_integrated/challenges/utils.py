...
def evaluate_challenge_success(challenge, nutrition_data, meal_records=None):
    ...
    Args:
        ...
        nutrition_data: 영양정보 딕셔너리 (예: {'total_calories': 1850, 'weight': 70, 'carbs': 200, 'protein': 80, 'fat': 50})
        meal_records: 식사 기록 리스트 (예: [{'meal_time': datetime, 'calories': 500}])

    Returns:
        dict: {'success': bool, 'actual_value': float, 'target_value': float, 'meal_validation': dict, 'message': str}
    """
    ...
    elif challenge.target_type == 'weight':
        actual_weight = nutrition_data.get('weight', 0)
        target_weight = float(challenge.target_value)
        # 목표 체중 이하로 도달하면 성공
        success = actual_weight <= target_weight
        message = f"목표: {target_weight}kg, 실제: {actual_weight}kg"
        return {
            'success': success,
    ...
    elif challenge.target_type == 'macro':
        # 예시: 단백질, 탄수화물, 지방 등 영양소 목표 (여기선 단백질만 예시)
        actual_protein = nutrition_data.get('protein', 0)
        target_protein = float(challenge.target_value)
        # 목표 단백질 이상 섭취하면 성공
        success = actual_protein >= target_protein
        message = f"목표: {target_protein}g 단백질, 실제: {actual_protein}g"
        return {
            'success': success,
            'actual_value': actual_protein,
